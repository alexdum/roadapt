...
extract_timeser_det <- function(tab,id, perio_sub, indic) {
  ...
  dd <- tab_sub |> mutate(
    ...
    change_med = case_when(
      indic %in% c("prAdjust", "ur") ~  (((p50*100)/mean(p50[an <= 2000])) - 100)  %>% round(1),
      !indic %in% c("prAdjust", "ur") ~ (p50 - mean(p50[an <= 2000]))  %>% round(1)),
    change_max = case_when(
      indic %in% c("prAdjust", "ur") ~  (((pmax*100)/mean(p50[an <= 2000])) - 100)  %>% round(1),
      !indic %in% c("prAdjust", "ur") ~ (pmax - mean(p50[an <= 2000]))  %>% round(1)),
    change_min = case_when(
      indic %in% c("prAdjust", "ur") ~  (((pmin*100)/mean(p50[an <= 2000])) - 100)  %>% round(1),
      !indic %in% c("prAdjust", "ur") ~ (pmin - mean(p50[an <= 2000]))  %>% round(1)),
